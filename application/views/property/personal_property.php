...
<table class="table table-striped" style="align:center; font-family:Microsoft JhengHei;">
        ...
        <?php foreach($personalPropertyList as $list => $value) : ?>
			...
			<td><?php echo $value['borrow_status']?></td>
                        ...
                        <td>
                        <button type="button" class="<?php echo $value['returnButtonStyle'];?>" <?php echo $value['extraInfo'];?> onclick="javascript:location.href='<?=base_url("/property/approve_app/")?><?php echo "/".$value['id']?>'"><?php echo $value['returnButtonString'];?></button>
                        &nbsp;
                        </td>
                </tr>
        <?php endforeach; ?>
